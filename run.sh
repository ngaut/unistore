...
sleep 2 
nohup ./node/node -store-addr=${host_ip}:9191 -L=info -sync-write=false -table-loading-mode=memory-map -max-table-size=300663296 -num-level-zero-tables=20 -region-size=6655443 -num-mem-tables=5 -value-threshold=2560 -db-path=data -pd-addr="http://${host_ip}:2379" &
sleep 2 
