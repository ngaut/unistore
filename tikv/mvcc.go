...
func (store *MVCCStore) Prewrite(reqCtx *requestCtx, mutations []*kvrpcpb.Mutation, primary []byte, startTS uint64, ttl uint64) []error {
	...
	for i, m := range mutations {
		...
		if !anyError {
			...
			lock := mvcccodec.MvccLock{
				MvccLockHdr: mvcccodec.MvccLockHdr{
					StartTS:    startTS,
					Op:         uint8(m.Op),
...
func (store *MVCCStore) checkPrewriteInLockStore(
	...
	req.buf = mvcccodec.EncodeRollbackKey(req.buf, mutation.Key, startTS)
	...
	lock := mvcccodec.DecodeLock(req.buf)
...
func (store *MVCCStore) checkPrewriteInDB(
	req *requestCtx, txn *badger.Txn, mutation *kvrpcpb.Mutation, startTS uint64) (userMeta mvcccodec.DBUserMeta, val []byte, err error) {
	...
	userMeta = mvcccodec.DBUserMeta(item.UserMeta())
...
func (store *MVCCStore) Commit(req *requestCtx, keys [][]byte, startTS, commitTS uint64) error {
	...
	userMeta := mvcccodec.NewDBUserMeta(startTS, commitTS)
	...
	for _, key := range keys {
		...
		if lock.HasOldVer {
			oldKey := mvcccodec.EncodeOldKey(key, lock.OldMeta.CommitTS())
...
func (store *MVCCStore) rollbackKeyReadLock(batch *writeLockBatch, key []byte, startTS uint64) (status int) {
	batch.buf = mvcccodec.EncodeRollbackKey(batch.buf, key, startTS)
	...
	if hasLock {
		lock := mvcccodec.DecodeLock(batch.buf)
		if lock.StartTS < startTS {
...
func (store *MVCCStore) rollbackKeyReadDB(req *requestCtx, batch *writeLockBatch, key []byte, startTS uint64, hasLock bool) error {
	...
	oldKey := mvcccodec.EncodeOldKey(key, userMeta.CommitTS())
	...
	for it.Seek(oldKey); it.ValidForPrefix(oldKey[:len(oldKey)-8]); it.Next() {
		...
		if mvcccodec.OldUserMeta(item.UserMeta()).StartTS() == startTS {
...
func checkLock(lock mvcccodec.MvccLock, key []byte, startTS uint64) error {
...
func (store *MVCCStore) CheckRangeLock(startTS uint64, startKey, endKey []byte) error {
	...
	for it.Seek(startKey); it.Valid(); it.Next() {
		...
		lock := mvcccodec.DecodeLock(it.Value())
...
func (store *MVCCStore) ResolveLock(reqCtx *requestCtx, startTS, commitTS uint64) error {
	...
	var userMeta mvcccodec.DBUserMeta
	if commitTS > 0 {
	...
	for i, lockKey := range lockKeys {
		...
		if bytes.Equal(buf, lockVals[i]) {
			if commitTS > 0 {
				lock := mvcccodec.DecodeLock(lockVals[i])
...
func (store *MVCCStore) DeleteRange(reqCtx *requestCtx, startKey, endKey []byte) error {
	...
	oldStartKey := mvcccodec.EncodeOldKey(startKey, maxSystemTS)
	oldEndKey := mvcccodec.EncodeOldKey(endKey, maxSystemTS)
...
func (f *GCCompactionFilter) Filter(key, value, userMeta []byte) badger.Decision {
	...
	case metaPrefix, tablePrefix:
		...
		if mvcccodec.DBUserMeta(userMeta).CommitTS() < f.safePoint && len(value) == 0 {
			return badger.DecisionMarkTombstone
		}
	case metaOldPrefix, tableOldPrefix:
		// The key is old version key.
		if mvcccodec.OldUserMeta(userMeta).NextCommitTS() < f.safePoint {
