...
func newStore() *store {
	...
	for i := range s.shards[:] {
		s.shards[i].data = make(map[Key]*item)
	}
	return s
}

func (s *store) GetValue(key Key) (interface{}, bool) {
	i, ok := s.Get(key)
	if !ok {
		return nil, false
	}
...
func (s *store) GetOrNew(key Key) *item {
	m := &s.shards[key.Hash()%numShards]
	m.RLock()
