...
func initialIngest(t *testing.T, db *Engine) {
...
func (sc *shardingCase) checkGet(snap *SnapAccess, begin, end int) {
	for i := begin; i < end; i++ {
		key := iToKey(i)
...
func (sc *shardingCase) checkIterator(snap *SnapAccess, begin, end int) {
	for cf := 0; cf < 2; cf++ {
...
func (sc *shardingCase) finishSplit(shardID, ver uint64, newIDs []uint64) {
	newProps := make([]*enginepb.Properties, len(newIDs))
	for i, newID := range newIDs {
		newProps[i] = &enginepb.Properties{ShardID: newID}
	}
...
func TestShardingMetaChangeListener(t *testing.T) {
	...
	snap := db.NewSnapAccess(db.GetShard(1))
...
func (l *metaListener) OnChange(e *enginepb.ChangeSet) {
	l.lock.Lock()
	if e.Flush != nil {
