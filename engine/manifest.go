...
func (m *Manifest) toChangeSets() ([]*enginepb.ChangeSet, error) {
	var shards []*enginepb.ChangeSet
...
func (m *Manifest) toChangeSet(shardID uint64) (*enginepb.ChangeSet, error) {
...
func (m *Manifest) ApplyChangeSet(cs *enginepb.ChangeSet) error {
	if cs.Snapshot != nil {
		m.applySnapshot(cs)
		return nil
	}
	...
	if cs.Flush != nil {
		...
		if cs.Stage == enginepb.SplitStage_PRE_SPLIT_FLUSH_DONE {
			shardInfo.splitStage = enginepb.SplitStage_PRE_SPLIT_FLUSH_DONE
	...
	if cs.PreSplit != nil {
		...
		shardInfo.splitStage = enginepb.SplitStage_PRE_SPLIT
		return nil
	}
	if cs.SplitFiles != nil {
		m.applySplitFiles(cs, shardInfo)
		shardInfo.splitStage = enginepb.SplitStage_SPLIT_FILE_DONE
		return nil
	}
	if cs.Split != nil {
...
func (m *Manifest) applySnapshot(cs *enginepb.ChangeSet) {
	...
	if len(cs.Snapshot.SplitKeys) > 0 {
		shard.preSplit = &enginepb.PreSplit{Keys: cs.Snapshot.SplitKeys}
	}
...
func (m *Manifest) applyFlush(cs *enginepb.ChangeSet, shardInfo *ShardMeta) {
...
func (m *Manifest) applyCompaction(cs *enginepb.ChangeSet, shardInfo *ShardMeta) {
...
func (m *Manifest) applySplitFiles(cs *enginepb.ChangeSet, shardInfo *ShardMeta) {
...
func (m *Manifest) applySplit(shardID uint64, split *enginepb.Split) {
...
func (m *Manifest) writeChangeSet(changeSet *enginepb.ChangeSet) error {
...
func (m *Manifest) isDuplicatedChange(change *enginepb.ChangeSet) bool {
	m.appendLock.Lock()
	defer m.appendLock.Unlock()
